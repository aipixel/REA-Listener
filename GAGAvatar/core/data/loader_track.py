...
# z modified
class DriverData_DynamicInput(torch.utils.data.Dataset):
    def __init__(self, driver_path, feature_data=None, point_plane_size=296):
        ...
        # build feature data
        print("feature_data is None?:", feature_data is None)
    ...
    def _load_one_record(self, frame_key):
        ...
        #print(self.f_image.size())
        t_image = self._lmdb_engine[frame_key].float() / 255.0
        #t_image = torch.zeros((3,512,512)).float()           #self._lmdb_engine[frame_key].float() / 255.0
        t_points = self.flame_model(
