...
from .human_matting import StyleMatteEngine as HumanMattingEngineOld
from .human_matting import NewMatteEngine as HumanMattingEngineNew
...
class CoreEngine:
    def __init__(self, focal_length, device='cuda',
        ...
        if matting_type == 'old':
            self.matting_engine = HumanMattingEngineOld(device=device)
        elif matting_type == 'new':
            self.matting_engine = HumanMattingEngineNew(device=device)
        else:
    ...
    def track_image(self, inp_images, inp_keys, if_matting=True):
        ...
        if not len(croped_images):
            print('No face detected in all the images, tracking failed. 1')
            return None
        ...
        if not len(base_results.keys()):
            print('No face detected in all the images, tracking failed. 2')
            return None
    ...
    def crop_image(self, inp_image):
        ...
        if bbox is None:
            return None
        #print("bbox before expand:", bbox)
        bbox = expand_bbox(bbox, scale=1.65).long()
        #print("bbox after expand:", bbox)
        ...
        crop_image_resized = torchvision.transforms.functional.resize(crop_image, (512, 512), antialias=True)
        return crop_image_resized#, crop_image, bbox
    
    def crop_image_test(self, inp_image):
        ori_height, ori_width = inp_image.shape[1:]
        # if not hasattr(self.emica_data_engine, 'insight_detector'):
        #     self.emica_data_engine._init_models()
        # bbox, _, _ = self.emica_data_engine.insight_detector.get(inp_image)
