...
import cv2
from core.libs.path_utils import resolve_mediapipe_segmenter_path


class NewMatteEngine(torch.nn.Module):
    def __init__(self, device='cpu'):
        super().__init__()
        self.seg_model = MediapipeSegmenter()
        self.segmenter = vision.ImageSegmenter.create_from_options(self.seg_model.options)
    
    @torch.no_grad()
    def forward(self, input_image, only_return_mask = False, **kwargs):
        if len(input_image.shape) == 4:
            input_image = input_image.squeeze(0)
        head_img_arr = input_image.cpu().detach().permute(1,2,0).numpy() * 255.0
        head_img_arr = head_img_arr.clip(0,255).astype(np.uint8)
        head_img_arr = cv2.cvtColor(head_img_arr, cv2.COLOR_RGB2BGR)

        segmap_head_img = self.seg_model._cal_seg_map(head_img_arr, segmenter = self.segmenter)
        ...
        """
        if only_return_mask:
            return segmap_head_img
        
        mat_mask =  torch.tensor((segmap_head_img[0,:,:]) <= 0.01).unsqueeze(0).float().to(input_image.device)  #(1,h,w)
        mat_mask = mat_mask.expand(3, -1, -1)
        return input_image.clone() * mat_mask
...
class MediapipeSegmenter:
    def __init__(self):
        model_path = resolve_mediapipe_segmenter_path()
