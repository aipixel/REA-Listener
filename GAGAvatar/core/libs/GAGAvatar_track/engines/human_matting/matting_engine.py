...
from core.libs.path_utils import resolve_track_asset_file
...
class StyleMatteEngine(torch.nn.Module):
    ...
    def _init_models(self, ):
        # load dict
        _ckpt_path = resolve_track_asset_file("matting/stylematte_synth.pt")
    ...
    @torch.no_grad()
    def forward(self, input_image, return_type='matting', background_rgb=1.0, bg_img = False):
        ...
        elif return_type == 'custom_z':         ##############
            predict = predict.expand(3, -1, -1)
            if bg_img:
                background_rgb = background_rgb # (c,h,w)
            else:
                background_rgb = input_image.new_ones(input_image.shape) * background_rgb
            foreground_image = input_image * predict + (1-predict) * background_rgb
            background_image = input_image * (1-predict) + predict * background_rgb
            return foreground_image, background_image, predict
        else:
            raise NotImplementedError
    
    @torch.no_grad()
    def forward_batch(self, input_image, return_type='matting', background_rgb=1.0, bg_img = False):
        if not hasattr(self, 'model'):
            self._init_models()
        if input_image.max() > 2.0:
            warnings.warn('Image should be normalized to [0, 1].')
        b, _, ori_h, ori_w = input_image.shape
        input_image = input_image.to(self._device).float()
        image = input_image.clone()
        # resize
        if max(ori_h, ori_w) > 1024:
        ...
        if return_type == 'alpha':
            ...
            predict = predict.expand(-1, 3, -1, -1)                                                 #b3hw
            if bg_img:
                background_rgb = background_rgb                                                     # (b,3,h,w)
            else:
                background_rgb = input_image.new_ones(input_image.shape) * background_rgb
            foreground_image = input_image * predict + (1-predict) * background_rgb
            background_image = input_image * (1-predict) + predict * background_rgb
