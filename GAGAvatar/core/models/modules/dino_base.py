...
from core.libs.path_utils import resolve_dinov2_pretrain_path, resolve_dinov2_repo
    ...
    def __init__(self, output_dim=128, only_global=False):
        ...
        try:
            self.dino_model = torch.hub.load(
                str(resolve_dinov2_repo()),
                "dinov2_vitb14",
                pretrained=False,
                source="local",
            )
            state_dict = torch.load(resolve_dinov2_pretrain_path(), map_location="cpu", weights_only=True)
            self.dino_model.load_state_dict(state_dict, strict=True)
        except Exception as exc:
            raise RuntimeError(
                "Failed to load DINOv2 from the vendored source and local checkpoint. "
                "Expected the source in GAGAvatar/third_party/dinov2 and the weight file "
                "dinov2_vitb14_pretrain.pth under GAGAvatar/checkpoints."
            ) from exc
