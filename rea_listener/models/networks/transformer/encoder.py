...
from .subsampling import EmbedinigNoSubsampling
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        elif pos_enc_layer_type == "ped_pos":
            pos_enc_class = PeriodicPositionalEncoding
...
class StylizationBlock(torch.nn.Module):
    ...
    def forward(self, h, emb):
        ...
        h = self.out_layers(h) + h
        return h
    
class TransformerEncoderWithStylizationBlock(BaseEncoder):
    """Transformer encoder module."""
    def __init__(
        self,
        input_size: int,
        ...
        self.stylization_blocks = torch.nn.ModuleList([
                StylizationBlock(output_size, style_feature_dim, dropout_rate) for _ in range(num_blocks)
        ])
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.encoders):
            xs, chunk_masks, _, _ = layer(xs, chunk_masks, pos_emb, mask_pad)
            xs = self.stylization_blocks[idx](xs, style_feature)
